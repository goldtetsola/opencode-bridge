...
DESKTOP_PROBE_PROVIDER_BLOCK = '''[model_providers.desktop_pre_final_text_probe]
...
[model_providers.desktop_pre_final_text_probe.auth]
command = "echo"
...
PROVIDER_BLOCK = (
    OPENCODE_PROVIDER_BLOCK.rstrip()
    + "\n\n"
    + OSS_RUNTIME_PROVIDER_BLOCK.rstrip()
    + "\n\n"
    + DESKTOP_PROBE_PROVIDER_BLOCK
)
...
AGENT_DESKTOP_PROBE = '''name = "desktop_pre_final_text_probe"
description = "Controlled Desktop renderer falsifier. USE ME ONLY WHEN testing whether Codex Desktop spawned-child views render ordinary assistant text before final. Does not use tools, OSS models, MissionV1, or repo files."

model_provider = "desktop_pre_final_text_probe"
model = "desktop-pre-final-text-probe"
model_reasoning_effort = "low"
sandbox_mode = "read-only"

developer_instructions = """
You are a renderer probe, not a worker.

Do not use tools.
Do not inspect files.
Do not perform repo work.
Do not summarize or transform the provider output.
Return exactly the streamed text you receive from the provider:
PROGRESS_ONE
PROGRESS_TWO
PROGRESS_THREE
FINAL_DONE

This agent cannot certify Desktop-native behavior by itself. A human or
ConsumerObservationWitnessV1 must record what the Desktop UI actually rendered.
"""
'''
...
def _ensure_config(root: Path, force: bool) -> int:
    ...
    if "[model_providers.desktop_pre_final_text_probe]" not in existing or force:
        provider_blocks.append(DESKTOP_PROBE_PROVIDER_BLOCK.strip())
...
def _ensure_agents(root: Path, force: bool) -> int:
    ...
    agents = {
        ...
        "desktop-pre-final-text-probe.toml": AGENT_DESKTOP_PROBE,
    }
